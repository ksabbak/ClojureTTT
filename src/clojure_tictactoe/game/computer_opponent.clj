...
(def memo-deduce-turn (memoize deduce-turn))

(defn get-turn-determiner [board]
  (if (even? (memo-deduce-turn board))
    even?
    odd?))
...
(declare mini-max memo-mini-max)
...
(def memo-calculate-scores (memoize calculate-scores))
